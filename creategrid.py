...
method=1 # 1: CloughTocher;  2: Polynomial
flatten=False
polydegree=2
...
def u(s,t):
  return 2*mHs-s-t
def beta(s):
  return math.sqrt(1.-4.*mHs/s)
def costh(s,t):
  return (t-u(s,t))/(s*beta(s))

def polyfit2d(x, y, z, orders):
...
class grid:
  def __init__(self,dim,xmin,xmax,nbin,data=[],binned=False):
    self.dim=dim
    ...
    if ( binned):
      ...
      for _ in range(nsamples):
        if nsamples > 1:
          dat=np.array([ np.append(d[:self.dim], [random.gauss(d[self.dim],d[self.dim+1]), d[self.dim+1]])  for d in data ]) # dat = data with y values sampled according to gauss(y,e)
        else:
          dat=data
        for k in np.ndindex(tuple(self.nbin)):
...
#          print "fitval: ",linfunc(x,*popt)
          self.data[k].add(popt[0],max(sqrt(pcov1[0,0]),sqrt(pcov2[0,0]))) # corresponds to multiplying error estimate of chi-sq fit with max(1,chisq)
          '''
             model y = f(x) = c
...
#      self.interpolators.append(interpolate.SmoothBivariateSpline(temp[0],temp[1],temp[2],bbox=[0.,1.,0.,1.]))
      ...
      for k in self.data:
        kk=np.asarray(k)
        kmin = np.maximum(kk - np.full(self.dim,nneighbor),np.zeros(self.dim)).astype(int)
        kmax = np.minimum(kk + np.full(self.dim,nneighbor),self.nbin-1).astype(int)
        degree = kmax-kmin
        self.deg[k]=degree

        xx=self.x(k)
        dat=[ np.append(self.x(x+kmin), self.data[tuple(x+kmin)].gety(sample=(nsamples!=1)))  for x in np.ndindex(*(degree+1))]
        x,y,z=np.asarray(zip(*dat))
#        z=np.asarray(zip(*dat)[2])
        self.pol[k,i] = polyfit2d(x,y,z,orders=degree)
...
#        pol=np.Poly
  ...
  def interpolate(self,x,y,selectsample=-1):
    if ( method == 2):
      k=self.k(tuple([x,y])) 
      temp = [ polyval2d(x,y,self.pol[k,i],self.deg[k]) for i in range(self.nsamples) ]
      #return (polyval2d(x,y,self.pol[k],self.deg[k]),0.)
    else:
...
#      return (np.mean(temp),np.std(temp))
  ...
  def __call__(self,X,Y,selectsample=-1):
    temp=np.array([X,Y]).T
    s=np.shape(X)
    temp= np.array([ self.interpolate(x,y,selectsample)[0] for x,y in  np.array([ x for x in np.vstack([X.ravel(), Y.ravel()]) ]).T ]).reshape(s)
...
cdfdata=np.loadtxt('events.cdf')
cdf=interpolate.splrep(*cdfdata.T,k=1)
...
#dataInUniform=np.loadtxt('grid.in',converters={0: lambda sbeta: interpolate.splev(float(sbeta),cdf)})

x0,x1,y,e=np.loadtxt('grid.in',unpack=True)
dataIn=np.array([x0,x1,y,e]).T

x0Uniform=interpolate.splev(x0,cdf)
dataInUniform=np.array([x0Uniform,x1,y,e]).T

if(flatten):
  poly= np.poly1d(np.polyfit(x0Uniform, y, polydegree,w=1./e))
...
a=grid(2,(0,0),(1,1),(100,30),data=dataInUniformFlat,binned=False)

#a.addPoint(dataInUniformFlat)
a.initInterpolation(1)

def GetAmplitude(s,t):
  b=beta(s)
  cTh=abs(costh(s,t))
  xUniform=interpolate.splev(b,cdf)
#  print s,t
#  print b,cTh
#  print a.interpolate(xUniform,cTh)[0]
  return a.interpolate(xUniform,cTh)[0]
...
#  #print temp
#  plt.show()
#  ##with np.loadtxt('grid.in') as data:
#  ##  for d in data:
#  #
