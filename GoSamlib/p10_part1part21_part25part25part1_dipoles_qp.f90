module     p10_part1part21_part25part25part1_dipoles_qp
   use p10_part1part21_part25part25part1_config, only: ki => ki_qp
   use p10_part1part21_part25part25part1_color_qp, only: gammaF, gammaA, &
       & CF, CA, numcs, KF, KA, &
       & T1T2, &
       & T1T5, &
       & T2T5
   use p10_part1part21_part25part25part1_kinematics_qp, only: num_legs, dotproduct, &
       lambda
   implicit none
...
contains
   ...
   function I_ff_qed(mu_sq, vec, I, J)
      ...
      if (flag) then
              ...
              term = V_SING_QED(s_jk, real(mdlMh, ki), real(0, ki))
              term(1) = (term(1) + log_s * term(2) ) * ncs*(0.0_ki)*(0.333333333333_ki)/9.0_ki
              term(2) = term(2)* ncs*(0.0_ki)*(0.333333333333_ki)/9.0_ki
              I_ff_qed(:,:,1) = I_ff_qed(:,:,1) + term(1) * matrix(:,:)
              I_ff_qed(:,:,2) = I_ff_qed(:,:,2) + term(2) * matrix(:,:)
      end if
   ...
   function I_if_qed(mu_sq, vec, I, J)
      ...
      if (flag) then
              ...
              term=term*ncs*(-0.333333333333_ki)*(0.0_ki/9.0_ki)
              term(1) = term(1) + log_s * term(2)
      ...
      if (flag) then
              ...
              term=term*ncs*(-0.333333333333_ki)*(0.333333333333_ki/9.0_ki)
              term(1) = term(1) + log_s * term(2)
      ...
      if (flag) then
              ncs=1.0_ki
              term = V_SING_QED(s_ja, 0.0_ki, 0.0_ki)
              term=term*ncs*(0.0_ki)*(0.333333333333_ki/9.0_ki)
              term(1) = term(1) + log_s * term(2)
   ...
   function I_ii_qed(mu_sq, vec, I, J)
      ...
      if (flag) then
              ...
              term(1) = ncs*(-0.333333333333_ki)*(0.0_ki)/9.0_ki
              term(2) = ncs*(-0.333333333333_ki)*(0.0_ki)/9.0_ki
...
end module p10_part1part21_part25part25part1_dipoles_qp
