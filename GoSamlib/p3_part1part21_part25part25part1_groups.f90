...
function     contract_tensor_coefficients_group_0(coeffs) result(amp)
   ...
   use p3_part1part21_part25part25part1_config, only: debug_nlo_diagrams, logfile
   use p3_part1part21_part25part25part1_kinematics, only: k1, k2, k3, k4, k5, es45, es12, es3, es51, es23, es4, es34
   ...
   s_mat(3,1)=s_mat(1,3)
   s_mat(1,4)=real(-2.0_ki*mdlMT**2+es51, ki_gol)
   s_mat(4,1)=s_mat(1,4)
   ...
   !-------#[ Diagram 7:
      ...
      amp = + (contract4_4(coeffs%coeffs_7, rmomenta, b_set))
      if(debug_nlo_diagrams) then
         dbg_amp = amp - dbg_amp
         write(logfile,100) &
...
function     contract_tensor_coefficients_group_1(coeffs) result(amp)
   ...
   !-------#[ Diagram 9:
      ...
      amp = amp + (contract4_4(coeffs%coeffs_9, rmomenta, b_set))
      if(debug_nlo_diagrams) then
         dbg_amp = amp - dbg_amp
         write(logfile,100) &
            & "<result kind='nlo-finite' re='", real(dbg_amp%C, ki), &
...
function     contract_tensor_coefficients_group_2(coeffs) result(amp)
   ...
   !-------#[ Diagram 11:
      ...
      amp = + (contract4_4(coeffs%coeffs_11, rmomenta, b_set))
      if(debug_nlo_diagrams) then
         dbg_amp = amp - dbg_amp
         write(logfile,100) &
            & "<result kind='nlo-finite' re='", real(dbg_amp%C, ki), &
