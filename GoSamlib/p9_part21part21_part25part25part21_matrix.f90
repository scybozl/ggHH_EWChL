module     p9_part21part21_part25part25part21_matrix
   use p9_part21part21_part25part25part21_util, only: square
   use p9_part21part21_part25part25part21_util_qp, only: square_qp => square
   ...
   use p9_part21part21_part25part25part21_model, only: Nf, NC, sqrt2, init_functions
   use p9_part21part21_part25part25part21_model_qp, only: Nf_qp => Nf, NC_qp => NC, sqrt2_qp => sqrt2, &
     & init_functions_qp => init_functions
   use p9_part21part21_part25part25part21_color, only: TR, CA, CF, numcs, &
     & incolors, init_color
   use p9_part21part21_part25part25part21_color_qp, only: TR_qp => TR, CA_qp => CA, CF_qp => CF, &
     & incolors_qp => incolors, init_color_qp => init_color
   use p9_part21part21_part25part25part21_diagramsh0l0, only: amplitude0l0 => amplitude
   use p9_part21part21_part25part25part21_diagramsh0l0_qp, only: amplitude0l0_qp => amplitude
   use p9_part21part21_part25part25part21_amplitudeh0, only: samplitudeh0l1 => samplitude, &
     &   finite_renormalisation0 => finite_renormalisation
   use p9_part21part21_part25part25part21_amplitudeh0_qp, only: samplitudeh0l1_qp => samplitude, &
     &   finite_renormalisation0_qp => finite_renormalisation
   use p9_part21part21_part25part25part21_dipoles, only: insertion_operator, insertion_operator_qed
   use p9_part21part21_part25part25part21_dipoles_qp, only: insertion_operator_qp => insertion_operator, &
...
contains
   ...
   subroutine     samplitude(vecs, scale2, amp, prec, ichecked, ok, h)
      use p9_part21part21_part25part25part21_kinematics_qp, only: adjust_kinematics_qp => adjust_kinematics
      ...
      fpprec2 = 18
      spprec2 = 20
      prec = 20 ! Set precision to unrealistically high value
      scales2(:) = (/0.0_ki, &
     &              mdlMh, &
     &              mdlMh/)
      ...
      ! RESCUE SYSTEM
      if(PSP_check) then
         ! If SM amplitude needed rescue, automatically rescue SMxBSM
         ...
         if(icheck.eq.3.and.PSP_verbosity) then
            ...
            write(42,'(4x,A15,A34,A3)') &
                 &  "<process name='","p9_part21part21_part25part25part21","'/>"
   ...
   subroutine     samplitudel01(vecs, scale2, amp, rat2, ok, h)
      ...
      use p9_part21part21_part25part25part21_model
      use p9_part21part21_part25part25part21_dipoles, only: pi
      implicit none
      real(ki), dimension(5, 4), intent(in) :: vecs
      real(ki), intent(in) :: scale2
      ...
      case default
         ! not implemented
         print*, "In p9_part21part21_part25part25part21_matrix:"
   ...
   function     samplitudel0(vecs, h) result(amp)
      use p9_part21part21_part25part25part21_config, only: logfile
      use p9_part21part21_part25part25part21_kinematics, only: init_event
      implicit none
      real(ki), dimension(5, 4), intent(in) :: vecs
   ...
   subroutine     samplitudel01_qp(vecs, scale2, amp, rat2, ok, h)
      ...
      use p9_part21part21_part25part25part21_dipoles_qp, only: pi
      implicit none
   ...
   function     samplitudel0_qp(vecs, h) result(amp)
      use p9_part21part21_part25part25part21_config, only: logfile
      use p9_part21part21_part25part25part21_kinematics_qp, only: init_event
      implicit none
      real(ki_qp), dimension(5, 4), intent(in) :: vecs
      integer, optional, intent(in) :: h
   ...
   pure subroutine color_correlated_lo(color_vector,res)
      use p9_part21part21_part25part25part21_color, only: T1T1, &
      & T1T2, &
      & T1T5, &
      & T2T2, &
      & T2T5, &
   ...
   subroutine spin_correlated_lo2(vecs, bornsc)
      ...
      call init_event(pvecs, +1, +1, +1)
      !---#] reinitialize kinematics:
      heli_amp7 = amplitude0l0()
      !---#] Initialize helicity amplitudes :
      !---#[ Initialize polarization vectors :
      eps1 = spvak2k1/Spaa(k2,k1)/sqrt2
      eps2 = spvak5k2/Spaa(k5,k2)/sqrt2
      eps5 = conjg(spvak2k5/Spaa(k2,k5)/sqrt2)
      !---#] Initialize polarization vectors :
      ...
      call construct_polarization_tensor(conjg(eps5),eps5,tens)
      bornsc(5,:,:) = bornsc(5,:,:) + real(tens(:,:) * pp, ki)
      call construct_polarization_tensor(conjg(eps5),conjg(eps5),tens)
      bornsc(5,:,:) = bornsc(5,:,:) + real(tens(:,:) * pm, ki)
      call construct_polarization_tensor(eps5,eps5,tens)
      bornsc(5,:,:) = bornsc(5,:,:) + real(tens(:,:) * mp, ki)
      call construct_polarization_tensor(eps5,conjg(eps5),tens)
      bornsc(5,:,:) = bornsc(5,:,:) + real(tens(:,:) * mm, ki)
      !---#] particle 5 :
   ...
   pure function square_0l_0l_sc(color_vector1, color_vector2) result(amp)
      use p9_part21part21_part25part25part21_color, only: cmat => CC
      implicit none
   ...
   pure function square_1_2_sc(color_vector1, color_vector2) result(amp)
      use p9_part21part21_part25part25part21_color, only: cmat => T1T2
   ...
   pure function square_1_5_sc(color_vector1, color_vector2) result(amp)
      use p9_part21part21_part25part25part21_color, only: cmat => T1T5
   ...
   pure function square_2_5_sc(color_vector1, color_vector2) result(amp)
      use p9_part21part21_part25part25part21_color, only: cmat => T2T5
...
end module p9_part21part21_part25part25part21_matrix
