...
contains
   ...
   pure function prefactor()
      ...
      use p1_dg_hhd_model, only: NC
      use p1_dg_hhd_kinematics, only: &
      & in_helicities, symmetry_factor
      use p1_dg_hhd_color, only: incolors
      implicit none
   ...
   subroutine     initgolem(is_first)
      use p1_dg_hhd_matrix, only: orig_initgolem => initgolem
      implicit none
   ...
   subroutine     exitgolem(is_last)
      use p1_dg_hhd_matrix, only: orig_exitgolem => exitgolem
      implicit none
   ...
   subroutine     samplitude(vecs, scale2, amp, prec, ok, h)
      use p1_dg_hhd_matrix, only: orig_sub => samplitude
      implicit none
   ...
   function     samplitudel0(vecs, h) result(amp)
      use p1_dg_hhd_matrix, only: orig_func => samplitudel0
      implicit none
      real(ki), dimension(5, 4), intent(in) :: vecs
   ...
   function     samplitudel1(vecs,scale2,ok,rat2,h) result(amp)
      use p1_dg_hhd_matrix, only: orig_func => samplitudel1
      implicit none
      real(ki), dimension(5, 4), intent(in) :: vecs
   ...
   subroutine OLP_color_correlated(vecs,ampcc)
      use p1_dg_hhd_matrix, only: orig_func => OLP_color_correlated
      implicit none
      real(ki), dimension(5, 4), intent(in) :: vecs
   ...
   subroutine OLP_spin_correlated_lo2(vecs, ampsc)
      use p1_dg_hhd_matrix, only: orig_func => OLP_spin_correlated_lo2
      implicit none
      real(ki), dimension(5, 4), intent(in) :: vecs
...
end module p4_gdbar_hhdbar_matrix
