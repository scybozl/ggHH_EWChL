module    pb_gg_hh_amplitudeh3
   use pb_gg_hh_config, only: ki, &
       & reduction_interoperation
   use pb_gg_hh_color, only: numcs
   use pb_gg_hh_groups
   use precision_golem, only: ki_gol => ki
   use pb_gg_hh_golem95h3
   use ninjago_module, only: ki_nin
   use pb_gg_hh_ninjah3
...
!---#[ function finite_renormalisation:
   function     finite_renormalisation(scale2) result(amp)
      use pb_gg_hh_util, only: square
      use pb_gg_hh_color, only: CF, CA
      use pb_gg_hh_kinematics, only: &
   ...
   function     samplitude(scale2,ok,rational2,the_col0,opt_perm)
      use pb_gg_hh_config, only: include_eps_terms, include_eps2_terms, &
      & logfile, debug_nlo_diagrams
      use pb_gg_hh_globalsl1, only: col0,perm, use_perm, epspow
      use pb_gg_hh_globalsh3, &
     & only: init_lo, rat2
      use pb_gg_hh_abbrevd10h3, only: init_abbrevd10 => init_abbrev
      use pb_gg_hh_abbrevd4h3, only: init_abbrevd4 => init_abbrev
      use pb_gg_hh_abbrevd8h3, only: init_abbrevd8 => init_abbrev
      use pb_gg_hh_abbrevd6h3, only: init_abbrevd6 => init_abbrev
      use pb_gg_hh_groups
      implicit none
      real(ki), intent(in) :: scale2
...
subroutine     evaluate_group0(scale2,samplitude,ok)
   use pb_gg_hh_config, only: &
      & logfile, debug_nlo_diagrams
   use pb_gg_hh_globalsl1, only: epspow
   use parametre, only: mu2_scale_par
   use form_factor_type, only: form_factor
   use pb_gg_hh_golem95h3, only: reconstruct_golem95 => reconstruct_group
   use pb_gg_hh_groups, only: contract_golem95
   use pb_gg_hh_ninjah3, only: ninja_reduce => ninja_reduce_group0
   implicit none
   real(ki), intent(in) :: scale2
   logical, intent(out) :: ok
...
subroutine     evaluate_group1(scale2,samplitude,ok)
   ...
   use pb_gg_hh_golem95h3, only: reconstruct_golem95 => reconstruct_group
   use pb_gg_hh_groups, only: contract_golem95
   use pb_gg_hh_ninjah3, only: ninja_reduce => ninja_reduce_group1
   implicit none
   real(ki), intent(in) :: scale2
   logical, intent(out) :: ok
   complex(ki), dimension(-2:0), intent(out) :: samplitude
...
subroutine     evaluate_group2(scale2,samplitude,ok)
   ...
   use pb_gg_hh_golem95h3, only: reconstruct_golem95 => reconstruct_group
   use pb_gg_hh_groups, only: contract_golem95
   use pb_gg_hh_ninjah3, only: ninja_reduce => ninja_reduce_group2
   implicit none
   real(ki), intent(in) :: scale2
   logical, intent(out) :: ok
   complex(ki), dimension(-2:0), intent(out) :: samplitude
...
!---#] subroutine evaluate_group2:
end module pb_gg_hh_amplitudeh3
