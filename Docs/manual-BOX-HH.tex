...
\POWHEGBOX{} fortran code. In order for the grid to be found by the code, the
...
The \POWHEGBOX{} offers the possibility to use a damping factor $h=${\tt hdamp} of the
...
\begin{itemize}
\item With {\tt mtdep=3}, an error message containing ``grid not found" appears: make sure your run folder contains the following files 
(or a symbolic link to these files located in the folder `Virtual'): {\tt events.cdf, creategrid.py} and the grid file  {\tt Virt\_full\_cHHH\_*.grid}, where {\tt cHHH} is the value for the trilinear Higgs coupling chosen in the input card {\tt powheg.input-save}.
...
\item After parallelstage 2, many of the *.dat files are empty: this seems to be a problem related to the {\tt gcc 6.3.0} compiler. It should not occur when using a more recent {\tt gcc} compiler.
\end{itemize}
