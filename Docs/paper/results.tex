...
We compare the \pythia~\cite{Sjostrand:2014zea} and \herwig~\cite{Bellm:2017bvx} parton showers, applied directly to the \powheg{} LHE events.
In the \herw case, we also compare the default shower (the angular-ordered $\tilde{q}-$shower) with the dipole shower.
In addition, we assess the uncertainties stemming from the matching and show results where the \herw parameter {\tt HardScale} is varied between $0.5$ and $2$. For all shower algorithms considered, the default tune of the corresponding version is used. Multiple-parton interactions (MPI) and hadronization are switched off. Finally, in the rest of the section, the \hdamp{} parameter in \powheg{} is set to $\hdamp=250$\,GeV.
