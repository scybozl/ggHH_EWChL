...
\end{subfigure}
\caption{The transverse momentum of one (any) Higgs boson and the angular distance between 
both Higgs bosons are shown for the fixed-order NLO calculation and three shower setups, in 
the $\chhh=1$ case.
\label{fig:lambdavar14TeV_pTH_dRHH_showers}}
\end{figure}

In general, observables that are inclusive in the additional radiation, like the average 
transverse momentum of one (any) Higgs $p_T^h$, show little sensitivity to the details of 
the parton showering, as highlighted in Fig.~\ref{fig:lambda_pTh}. In contrast, Fig.~\ref{fig:lambda_dRHH} 
shows the distribution of the angular distance $\Delta R^{\mathrm{hh}}$ between both Higgs 
...
and all three shower setups at 14\,TeV for $\chhh=1$, $\chhh=2.4$.
...
agree very well in the small-$\pthh$ range, but start to deviate at $\pthh \sim 100$\,GeV. 
Where both \herw showers converge back to the fixed-order calculation at high-$\pthh$, the 
\pyth shower produces much harder additional radiation and the ratio to the fixed-order result 
stagnates at $\sim 2.0$ over the remaining range. On the other hand, the \herw $\tilde{q}$- 
and dipole showers give very similar results. 
Specifically, in the \herw default shower, systematic uncertainties can be estimated by 
varying the maximal transverse momentum allowed for shower emissions $\mu_Q$, the so-called 
hard scale. We apply a factor $c_Q=\{0.5,\,2.0\}$ on the shower central hard scale separately 
...
uncertainty would be of the order of 50\% in these bins. It will be enlightening to study 
parton-shower (and non-perturbative) effects in the particular context of Higgs boson pair 
production at NLO, and try to reduce discrepancies among the different algorithms. 


\begin{figure}[htb]
 \begin{subfigure}[t]{0.495\textwidth}
\includegraphics[width=\textwidth]{plots/{PH7_PH7QUp_PH7QDown_cHHH_1_ptHH}.pdf}
 \caption{\label{fig:lambda_ptHH_muQvar}}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.495\textwidth}
    \includegraphics[width=\textwidth]{plots/{PH7_PH7QUp_PH7QDown_cHHH_1_dRHH}.pdf}
\caption{\label{fig:lambda_dRHH_muQvar}}\end{subfigure}
\caption{Higgs boson pair transverse momentum and relative angular distance for variations of the \herw $\tilde{q}$-shower hard scale.
